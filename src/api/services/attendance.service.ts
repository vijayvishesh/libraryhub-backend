...
export class AttendanceService {
  ...
  public async checkOut(id: string, studentId: string): Promise<AttendanceRecord> {
    ...
    if (!updated) {
      throw new NotFoundError('ATTENDANCE_NOT_FOUND');
    }
    return updated;
  }
  ...
  public async getTodayAttendanceForOwner(ownerId: string): Promise<{
    ...
    const records = await this.attendanceRepository.findTodayByLibrary(library.id, today);
    ...
    const absent = activeMembers.filter(
      m => m.studentId && !checkedInStudentIds.has(m.studentId),
    ).length;

    return { records, present, onBreak, absent };
  }

  private async getAttendanceById(id: string, studentId: string): Promise<AttendanceRecord> {
  ...
  public async getStudentAttendanceById(
    ...
    const weeklyPercentage =
      weeklyWorkingDays > 0 ? Math.round((weekPresentDays / weeklyWorkingDays) * 100) : 0;
    ...
    const monthlyPercentage =
      monthlyWorkingDays > 0 ? Math.round((monthPresentDays / monthlyWorkingDays) * 100) : 0;
